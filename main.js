...
class SoundCloudPlayer {
  ...
  loadTrack(e) {
    ...
    if (this.currentTrackId === trackId && !this.player.hidden) {
      this.close();
      return;
    }
    ...
    this.playButtons.forEach((btn) => {
      btn.classList.toggle('is-playing', btn.dataset.trackId === trackId);
    });

    // Load iframe with minimal UI
    this.iframe.src = `https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/${trackId}&color=${CONFIG.soundcloud.color}&auto_play=true&hide_related=true&show_comments=false&show_user=false&show_reposts=false&show_teaser=false&visual=false&buying=false&sharing=false&download=false`;
